...
ll b[N],a[N],c[N];
const ll B=10000000;
const ll Maxn=500000,Maxq=500000;
	...
	for(ll i_=0;i_<Shu;i_++){
		...
		while(1){
			...
			if(!check()){
				...
				printf("%lld %lld\n",n,q);
				ll k=random(cnt3/3,(2*cnt3)/3);
				ll cnt=0;
				for(ll i=1;i<=cnt1;i++)c[i]=i;
				cnt=cnt1;
				for(ll i=1;i<=k;i++)c[++cnt]=i+B+B;
				shuffle(c+1,c+cnt+1,rng);
				
				ll _cnt=cnt;
				for(ll i=1;i<=cnt2;i++)c[++cnt]=i+B;
				for(ll i=k+1;i<=cnt3;i++)c[++cnt]=i+B+B;
				shuffle(c+_cnt+1,c+cnt+1,rng);
				for(ll i=1;i<=q;i++){
					ll x=c[i];
					if(x<B){
						printf("0 %lld %lld 0\n",a1[x].l,a1[x].r);
